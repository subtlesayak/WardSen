...
describe("release engineering guardrails", () => {
  ...
  it("uses npm's script entrypoint when generating SBOM artifacts", () => {
    ...
    expect(packageJson.overrides["fast-uri"]).toBe("3.1.7");
    expect(Object.keys(packageJson.overrides["fast-json-stringify"]).sort()).toEqual(["ajv", "fast-uri"]);
    expect(packageJson.overrides["fast-json-stringify"]["fast-uri"]).toBe("4.1.4");
    expect(packageJson.overrides["fast-json-stringify"].ajv["fast-uri"]).toBe("3.1.7");
  });
