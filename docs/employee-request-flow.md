...
7. The employee opens the separate Employee Portal route (`?view=employee`), which contains no admin queue, vault configuration or raw credential display.
...
12. The employee chooses the required credential, adds a reason, optional ticket and expected duration, then submits the request. For true emergencies, the employee can mark the request as break-glass, add a separate justification and confirm the audit impact.
...
18. For automated providers with sender-visible telemetry, WardSen refreshes provider status to show link access signals such as `active`, `viewed`, `limit_reached`, `expired` or `revoked`. For Ente Paste manual handoff, WardSen shows `handoff_pending` and cannot verify whether the Ente link was viewed.
...
Break-glass requests use the `break_glass` status and require the exact server confirmation phrase `BREAK GLASS <catalogEntryId>`, `confirmRiskSummary: true` and a justification of at least 12 characters. Break-glass does not show or deliver a credential by itself; it marks the request as emergency access and keeps admin fulfillment, denial, delivery and replacement workflows auditable.
...
- Do not let break-glass bypass server confirmation, emergency justification or admin delivery fulfillment.
