...
`v0.1.0-rc.54` is the latest published unsigned prerelease with Windows x64 and macOS Apple Silicon installer artifacts attached. Treat unsigned artifacts as validation builds, not fully trusted end-user releases. The next trusted release remains blocked on Windows Authenticode signing plus macOS Developer ID signing and notarization.
...
Use this path when preparing the next signed update after the unsigned `v0.1.0-rc.54` validation release.
