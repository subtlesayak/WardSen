...
`v0.1.0-rc.23` is published as an unsigned prerelease with Windows x64 and macOS Apple Silicon installer artifacts attached. Treat it as a validation build, not a fully trusted end-user release.
...
Use this path when preparing a signed update after the unsigned `v0.1.0-rc.23` validation release.
...
1. Create or choose the next release candidate tag, such as `v0.1.0-rc.23`.
...
5. Enter a tag such as `v0.1.0-rc.23`.
