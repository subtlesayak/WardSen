...
- Bitwarden Send: Send payloads and access passwords are redacted from command output, and disabled or expired sends map to WardSen delivery states.
- KeePassXC: local database path and optional key-file path are retained only as account unlock context; the database password is never written to SQLite.
