...
`v0.11.1` is the current stable release. The attached Windows MSI and macOS Apple Silicon DMG are unsigned; verify the matching checksum before installation and expect Windows SmartScreen, Defender, or macOS Gatekeeper warnings until code signing and notarization are configured.
...
See the [current release notes](docs/release-notes/v0.11.1.md), [security design](docs/security-design.md), and [installer signing guide](docs/installer-signing.md) for detail.
