...
`v0.1.0-rc.19` is the latest installer prerelease. It is suitable for developer review, security review and platform packaging validation.
...
- macOS Intel: not attached to `v0.1.0-rc.19`; maintainers can build it from the manual Intel workflow
...
`v0.1.0-rc.19` is intentionally published as an unsigned prerelease. To publish a trusted release later, maintainers need Windows Authenticode signing for `.exe` and `.msi` files, plus Apple Developer ID signing and notarization for macOS `.dmg` files.
