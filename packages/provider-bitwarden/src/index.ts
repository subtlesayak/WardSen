...
export class BitwardenCredentialProvider implements CredentialProvider {
  ...
  private powerShellTerminalLoginCommand(accountId: string, input: ProviderLoginInput, handoff: TerminalSessionHandoff) {
    ...
    return `$env:BITWARDENCLI_APPDATA_DIR=${profilePath}; $bwCommand=${command}; $bwSession=$null; try { if (-not $bwCommand -or -not (Test-Path -LiteralPath $bwCommand -PathType Leaf)) { throw "WardSen could not find the Bitwarden CLI. Install bw, then retry Terminal login / unlock." }; & $bwCommand config server ${serverUrl}; if ($LASTEXITCODE -ne 0) { throw "WardSen could not set the Bitwarden server." }; $bwStatusJson=& $bwCommand status --nointeraction 2>$null; $bwStatus=$null; try { $bwStatus=($bwStatusJson | ConvertFrom-Json -ErrorAction Stop).status } catch {}; if ($bwStatus -eq "unauthenticated") { ${firstLogin} } elseif ($bwStatus -eq "locked" -or $bwStatus -eq "unlocked") { Write-Host "WardSen will now unlock this Bitwarden profile. Type your Bitwarden master password in the Bitwarden prompt:"; $bwSession=& $bwCommand unlock --raw; $bwExitCode=$LASTEXITCODE } else { throw "WardSen could not read the Bitwarden CLI state. Close this terminal, restart WardSen, then retry Terminal login / unlock." }; if ($bwExitCode -ne 0 -or -not $bwSession) { throw "Bitwarden did not provide an unlock session. Verify the selected account email and server, then enter the master password that unlocks that account in Bitwarden. If it works in Bitwarden but fails here again, restart WardSen and retry." }; Invoke-WebRequest -Uri ${claimUrl} -Method Post -ContentType 'text/plain; charset=utf-8' -Headers @{ 'X-WardSen-Terminal-Handoff' = ${token} } -Body $bwSession.Trim() | Out-Null; Write-Host "WardSen received the Bitwarden session. Return to WardSen; this account is unlocked." } finally { $bwSession=$null; $bwStatusJson=$null; Remove-Item Env:\\BITWARDENCLI_APPDATA_DIR -ErrorAction SilentlyContinue }`;
  }
