...
export class BitwardenCredentialProvider implements CredentialProvider {
  ...
  private powerShellTerminalLoginCommand(accountId: string, input: ProviderLoginInput, handoff: TerminalSessionHandoff) {
    ...
    const serverUrl = powershellSingleQuote(input.serverUrl?.trim() || "https://vault.bitwarden.com");
    const firstLogin = input.sso
      ? `Write-Host "Bitwarden will open its SSO sign-in flow. Complete it, then use the Bitwarden password prompt below:"; & $bwCommand login --sso; $bwLoginExitCode=$LASTEXITCODE; if ($bwLoginExitCode -ne 0) { throw "WardSen could not finish Bitwarden SSO login. Fix the Bitwarden CLI message above, then retry." }; Write-Host "WardSen will now unlock this Bitwarden profile. Type your Bitwarden master password in the Bitwarden prompt:"; $bwSession=& $bwCommand unlock --raw; $bwExitCode=$LASTEXITCODE`
      : `Write-Host "Bitwarden will request your master password once to sign in and unlock this isolated WardSen profile:"; $bwSession=& $bwCommand login${username} --raw; $bwExitCode=$LASTEXITCODE`;
    ...
    return `$env:BITWARDENCLI_APPDATA_DIR=${profilePath}; $bwCommand=${command}; $bwDesiredServer=${serverUrl}; $bwSession=$null; try { if (-not $bwCommand -or -not (Test-Path -LiteralPath $bwCommand -PathType Leaf)) { throw "WardSen could not find the Bitwarden CLI. Install bw, then retry Terminal login / unlock." }; $bwStatusJson=& $bwCommand status --nointeraction 2>$null; $bwStatus=$null; try { $bwStatus=($bwStatusJson | ConvertFrom-Json -ErrorAction Stop).status } catch {}; if ($bwStatus -eq "unauthenticated") { & $bwCommand config server $bwDesiredServer; if ($LASTEXITCODE -ne 0) { throw "WardSen could not set the Bitwarden server. If this profile is already logged in, select Unlock again instead of starting a new login." }; ${firstLogin} } elseif ($bwStatus -eq "locked" -or $bwStatus -eq "unlocked") { Write-Host "WardSen will now unlock this Bitwarden profile. Type your Bitwarden master password in the Bitwarden prompt:"; $bwSession=& $bwCommand unlock --raw; $bwExitCode=$LASTEXITCODE } else { throw "WardSen could not read the Bitwarden CLI state. Close this terminal, restart WardSen, then retry Terminal login / unlock." }; if ($bwExitCode -ne 0 -or -not $bwSession) { throw "Bitwarden did not provide an unlock session. Verify the selected account email and server, then enter the master password that unlocks that account in Bitwarden. If it works in Bitwarden but fails here again, restart WardSen and retry." }; Invoke-WebRequest -Uri ${claimUrl} -Method Post -ContentType 'text/plain; charset=utf-8' -Headers @{ 'X-WardSen-Terminal-Handoff' = ${token} } -Body $bwSession.Trim() | Out-Null; Write-Host "WardSen received the Bitwarden session. Return to WardSen; this account is unlocked." } finally { $bwSession=$null; $bwStatusJson=$null; Remove-Item Env:\\BITWARDENCLI_APPDATA_DIR -ErrorAction SilentlyContinue }`;
  }
  ...
  private posixTerminalLoginCommand(accountId: string, input: ProviderLoginInput, handoff: TerminalSessionHandoff) {
    ...
    const serverUrl = posixSingleQuote(input.serverUrl?.trim() || "https://vault.bitwarden.com");
    const firstLogin = input.sso
      ? `printf '\\nBitwarden will open its SSO sign-in flow. Complete it, then use the Bitwarden password prompt below:\\n'; "$bwCommand" login --sso; bwLoginExitCode=$?; if [ "$bwLoginExitCode" -eq 0 ]; then printf '\\nWardSen will now unlock this Bitwarden profile. Type your Bitwarden master password in the Bitwarden prompt:\\n'; bwSession="$("$bwCommand" unlock --raw)"; bwExitCode=$?; else bwExitCode=$bwLoginExitCode; fi`
      : `printf '\\nBitwarden will request your master password once to sign in and unlock this isolated WardSen profile:\\n'; bwSession="$("$bwCommand" login${username} --raw)"; bwExitCode=$?`;
    ...
    return `export BITWARDENCLI_APPDATA_DIR=${profilePath}; mkdir -p "$BITWARDENCLI_APPDATA_DIR"; ${bitwardenPosixCommandAssignment(this.executable())}; bwDesiredServer=${serverUrl}; bwSession=""; if [ -z "$bwCommand" ] || [ ! -x "$bwCommand" ]; then printf '\\nWardSen could not find the Bitwarden CLI. Install bw or put it at $HOME/.local/bin/bw, /opt/homebrew/bin/bw, /usr/local/bin/bw, /opt/local/bin/bw or $HOME/Library/Application Support/WardSen/tools/bw, then retry Terminal login / unlock.\\n' >&2; bwCanUnlock=0; else bwCanUnlock=1; fi; if [ "$bwCanUnlock" -eq 1 ]; then bwStatus="$("$bwCommand" status --nointeraction 2>/dev/null || true)"; if printf '%s' "$bwStatus" | grep -Eq '"status"[[:space:]]*:[[:space:]]*"unauthenticated"'; then "$bwCommand" config server "$bwDesiredServer" && { ${firstLogin}; } || { printf '\\nWardSen could not set the Bitwarden server. If this profile is already logged in, select Unlock again instead of starting a new login.\\n' >&2; bwExitCode=1; }; elif printf '%s' "$bwStatus" | grep -Eq '"status"[[:space:]]*:[[:space:]]*"(locked|unlocked)"'; then printf '\\nWardSen will now unlock this Bitwarden profile. Type your Bitwarden master password in the Bitwarden prompt:\\n'; bwSession="$("$bwCommand" unlock --raw)"; bwExitCode=$?; else printf '\\nWardSen could not read the Bitwarden CLI state. Close this terminal, restart WardSen, then retry Terminal login / unlock.\\n' >&2; bwExitCode=1; fi; else bwExitCode=127; fi; if [ "$bwExitCode" -eq 0 ] && [ -n "$bwSession" ]; then if printf '%s' "$bwSession" | curl --fail --silent --show-error --request POST --header 'Content-Type: text/plain; charset=utf-8' --header ${claimHeader} --data-binary @- ${claimUrl}; then printf '\\nWardSen received the Bitwarden session. Return to WardSen; this account is unlocked.\\n'; else printf '\\nWardSen could not transfer the Bitwarden session. Start Terminal login / unlock again from WardSen before retrying.\\n' >&2; fi; else if [ "$bwCanUnlock" -eq 1 ]; then printf '\\nBitwarden did not provide an unlock session. Verify the selected account email and server, then enter the master password that unlocks that account in Bitwarden. If it works in Bitwarden but fails here again, restart WardSen and retry.\\n' >&2; fi; fi; unset BITWARDENCLI_APPDATA_DIR bwSession bwExitCode bwLoginExitCode bwCanUnlock bwCommand bwCandidate bwStatus bwDesiredServer`;
  }
